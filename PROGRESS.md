...
**2026-08-12 — PwC programme select "selected then vanished": type-filter race + vacuous commit check, v0.3.8 → GREEN (exit 0, 151 passed, 1 skipped)**
- **User report:** programme_interest ("Winter Intern") errored on the live PwC form while every other dropdown filled. Server side verified clean first: label (now "…interested in-HKMO-FY27…") classifies right, saved answer exact-matches the harvested options, maxlength=-1 already guarded. So the failure was in the driving.
- **Root cause (user confirmed "it selected, then vanished"):** three compounding driver holes. (1) Typing the filter re-renders the MokaHR menu — the clicked option node can be DETACHED by the re-render, so the click commits nothing. (2) The commit check accepted `el.value === value` — but that's OUR OWN typed filter text, so a missed click still read as success (the "selection" the user watched was the filter text). (3) Exit hygiene then cleared that text — the visible vanish — leaving the field empty with a green-then-red outcome.
...
**2026-08-12 — Demo polish: cached PwC generation with staged progress bar, structured JD view, emoji sweep, extension → Vercel, v0.3.9 → GREEN (exit 0, 152 passed, 1 skipped)**
- **Demo generation cache (API):** new `demo_generation_cache` table (migration 009) + runner replays cached content instead of the LLM, paced ~1.8s (`DEMO_GEN_DELAY_MS`) so it doesn't look broken on stage — same trick as the extension's `demoPace`. Seeded for the PwC "Consulting - Cloud, Data & AI Consulting" job from ONE real DeepSeek generation each (resume 94s, cover 46s) via new `scripts/seed-demo-cache.ts`. Verified live: cached runs succeed in ~2.1s with byte-identical content; documents then wiped so the demo starts pristine ("Generate Resume" → ~3s staged bar → viewer auto-opens the finished PDF). Regression test added (cache sentinel replayed, LLM untouched).
- **GenerationCard rebuilt (web):** staged progress bar with eased fill and per-type stage labels, holds at 95% for real runs; on success auto-opens the DocumentViewer. Poll tightened 1500→700ms.
- **Structured JD view (web):** `JdView` renders light markdown (## headings, - bullets, | key | value | overview table); plain-text JDs unchanged. PwC demo JD rewritten as clean markdown (overview table + trimmed bullets, 2232 chars) via live PATCH.
- **Emoji sweep:** all colorful pictographs removed from web (nav 💼📄⚡, dialogs 🌐✏️, cards 📮📎✨🗑⬇) and extension (📄✉️📋⏳⚡). Monochrome typographic glyphs (✓ ⚠ ↗ ← ✕ ⠿) kept — standard UI iconography.
- **Extension → hosted UI:** webUrl default now `https://hireyou-apply.vercel.app`, and a stored legacy `localhost:5180`/`127.0.0.1:5180` value is treated as unset (the settings form used to persist the old default alongside the token) — "View in HireYou" now lands on Vercel. v0.3.8→**0.3.9** — user must reload the unpacked extension.
- API restarted with new code; web redeployed via push.
